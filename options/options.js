...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const data = await storage.get([GEMINI_API_KEY_KEY, GEMINI_MODEL_KEY]);
  ...
  document.getElementById("jjg-save-btn").addEventListener("click", async () => {
    ...
    await storage.set({ [GEMINI_API_KEY_KEY]: geminiKey, [GEMINI_MODEL_KEY]: geminiModel });
